...
function TargetCharms_UpdateRaidMarkerButtons()
    ...
    local frameName = frameNames[1];
    local host = _G[frameName];
    if not host then
        return;
    end
    local canSetAttr = not InCombatLockdown();
    if not canSetAttr then
        host:RegisterEvent("PLAYER_REGEN_ENABLED");
    end
    local systemOn = IsRaidMarkerSystemEnabled();
    for i = 1, 20 do
        local btn = _G[frameName .. "Charm" .. i];
        if btn and btn:IsShown() and buttonCharm[frameName][i] ~= nil and buttonCharm[frameName][i] >= 0 then
            local id = buttonCharm[frameName][i];
            local tex = _G[btn:GetName() .. "CharmTex"];
            if systemOn then
                if canSetAttr then
                    btn:SetAttribute("macrotext", "/tm " .. id);
                end
                if tex then
                ...
                btn:SetScript("OnEnter", function(self)
                    ...
                    GameTooltip:AddLine(TARGETCHARMS_RAID_MARKERS_UNAVAILABLE, 1, 1, 1, true);
                    GameTooltip:Show();
                end);
...
function TargetCharms_Command(msg)
    ...
    local _, _, cmd = string.find(msg, "(%S+)%s*(%S*)")
...
function TargetCharms_OnEvent(self, event)
    ...
    if event == "PLAYER_ENTERING_WORLD" or event == "ZONE_CHANGED_NEW_AREA" then
        TargetCharms_UpdateRaidMarkerButtons();
    end
...
function TargetCharms_Reset()
    ...
    if (tmpFrame ~= nil) then
        tmpFrame:ClearAllPoints();
        tmpFrame:SetPoint("CENTER", UIParent, "CENTER", 0, 0);
    end
    UpdateGlobal();
    TargetCharms_UpdateRaidMarkerButtons();
end
...
function FormatButton(frame, buttonNum, posChar, typeNum, xSpacing, ySpacing)
    ...
    if frame == frameNames[1] then
        ...
        if charmId ~= nil and charmId >= 0 then
            button:SetAttribute("type", "macro")
...
function ShowSetup()
    ...
    if setupFrame == nil then
        ...
        for i, v in ipairs(TargetCharms_LayoutDefaults) do
            local button = CreateFrame("Button", "ButtonPresetOptions" .. i, setupFrame, "PresetOptionsTemplate");
            button:SetID(i);
        ...
        for i, v in ipairs(Flare_LayoutDefaults) do
            local button = CreateFrame("Button", "ButtonFlarePresetOptions" .. i, setupFrame, "FlarePresetOptionsTemplate");
            button:SetID(i);
            _G[button:GetName() .. "Text"]:SetText(i);
            button:SetPoint("TOP", setupFrame, "TOP", (setupFrame:GetLeft() - button:GetLeft() + 22 * i), button:GetTop() - setupFrame:GetTop());
        end
